...
select kh.ma_khach_hang, kh.ho_ten, lk.ten_loai_khach, count(kh.ho_ten) as "so_lan_dat_phong" 
...
select month(ngay_lam_hop_dong) as tháng, count(ma_hop_dong) as "so_luong_khach_hang" from hop_dong
...
where (month(hd.ngay_lam_hop_dong) >= 10 and year(hd.ngay_lam_hop_dong) = 2020) and 
		dv.ma_dich_vu not in(select hd.ma_dich_vu from hop_dong hd
...
where ma_nhan_vien not in (select * from
	(
...
where ma_khach_hang in (select * from 
	(
		...
		having sum(ifnull(dv.chi_phi_thue, 0) + ifnull(hdct.so_luong, 0) * ifnull(dvdk.gia, 0)) > 1000000
    )
temp);
